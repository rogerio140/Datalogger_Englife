...
void checkMotor() {
  if (Firebase.getInt(firebaseData, "/motor")) {
    ...
    // Controle manual via Firebase
    if(novo_estado == 1 && motor_state == 0) {
      controlarMotor(tempo_acionamento);  // Aciona por 10 segundos
      motor_state = 1;       // Atualiza estado global
      Serial.println("[MOTOR] Acionamento manual por 10 segundos");
      
      // Atualiza Firebase para evitar retrigger
  ...
  if(millis() - ultimaVerificacao >= intervalo) {
    if(Firebase.getFloat(firebaseData, "/controle/peso")) {
      ...
      if(pesoDesejado > 0) {
        ...
        //tempo = max(tempo, 0.1); // Tempo mínimo de 0.1 segundos
