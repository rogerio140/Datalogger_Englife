...
void mostrarUltimas24h(const String& nomeArquivo, int intervaloMinutos) {
    ...
    TimeSpan intervalo(intervaloMinutos * 60); // Converte minutos para segundos

    // Definindo o limite de tempo com base no intervalo fornecido
    DateTime limite = agora - intervalo;
    
    Serial.print("Registros das últimas ");
    Serial.print(intervaloMinutos);
...
void enviarDadosFirebase(const String& linha) {
    int dia, mes, ano, hora, minuto, segundo;
    float tempAgua, tempEstufa, tempExterna;
    ...
    if (sscanf(linha.c_str(), "%d/%d/%d\t%d:%d:%d\t%f\t%f\t%f", 
        ...
        String dataHora = String(ano) + "-" + (mes < 10 ? "0" : "") + String(mes) + "-" + (dia < 10 ? "0" : "") + String(dia) +
                          "_" + (hora < 10 ? "0" : "") + String(hora) + "-" + (minuto < 10 ? "0" : "") + String(minuto) + "-" + (segundo < 10 ? "0" : "") + String(segundo);
        
        // Criar os caminhos para enviar ao Firebase
        String caminhoTemperaturaAgua = "/temperaturas/" + dataHora + "/agua";
        String caminhoTemperaturaEstufa = "/temperaturas/" + dataHora + "/estufa";
        String caminhoTemperaturaExterna = "/temperaturas/" + dataHora + "/externa";
...
void setup() {
    ...
    mostrarUltimas24h(nomeArquivo,intervaloMinutos); // Adicionado o ponto e vírgula correto
    ultimaColeta = rtc.now();
}
void loop() {
    
    if (verificarHorarioColetaCustom(tempoPersonalizado)) {
    Serial.println("Hora de realizar uma nova coleta!");
          if (WiFi.status() == WL_CONNECTED) {
          enviarLeituras() ;
          }
    ultimoEnvio = rtc.now(); // Atualiza o horário do último envio
    } 

    
    // Verificar se é hora de uma nova coleta e realizar leitura
    if (verificarHorarioColeta() && rtc.now() != ultimaColeta) {
        registrarLeituras();
        if (WiFi.status() != WL_CONNECTED) {
